...
from GroovyRobot import BOT_ID
from GroovyRobot import DRAGONS as DEVS
from GroovyRobot import OWNER_ID
from GroovyRobot import telethn as Groovy
from GroovyRobot.events import Groovyinline
from GroovyRobot.events import register as Groovybot
from GroovyRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Groovy(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Groovy(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Groovybot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
*ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ:*
   ...
   •➥ *ᴀʟᴏɴᴇ ʀᴏʙᴏᴛ ᴄᴀɴ ᴍᴜᴛᴇ ᴍᴇᴍʙᴇʀꜱ ᴡʜᴏ ᴀʀᴇ ɴᴏᴛ ꜱᴜʙꜱᴄʀɪʙᴇᴅ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴜɴᴛɪʟ ᴛʜᴇʏ ꜱᴜʙꜱᴄʀɪʙᴇ*
   ...
   •➥ *ꜱᴇᴛᴜᴘ*
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/ADVANCE_XD_BOT?startgroup=new)
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ your ᴄʜᴀɴɴᴇʟ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/ADVANCE_XD_BOT?startgroup=new)
