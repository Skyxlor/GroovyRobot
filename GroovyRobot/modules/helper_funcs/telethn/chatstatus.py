...
from GroovyRobot import DRAGONS
from GroovyRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def groovy_is_admin(chat_id: int):
    status = False
    groovy = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if groovy.id == user.id:
            status = True
            break
    return status
